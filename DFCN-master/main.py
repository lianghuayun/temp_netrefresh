...
if opt.args.name == 'usps':
    ...
    opt.args.n_clusters = 6
    opt.args.n_input = 100
elif opt.args.name == 'kyber':
    opt.args.k = None
    opt.args.n_clusters = 5
    opt.args.n_input = 50
elif opt.args.name == 'kyberpublic':
    opt.args.k = None
    opt.args.n_clusters = 5
    opt.args.n_input = 50
elif opt.args.name == 'kyberpublictemp':
    opt.args.k = None
    opt.args.n_clusters = 3
    opt.args.n_input = 50
else:
...
graph = ['acm', 'dblp', 'cite']
non_graph = ['usps', 'hhar', 'reut', 'kyber', 'kyberstft', 'kyberpublic', 'kyberpublictemp']
...
x_temp = x[0]
scattering=Scattering1D(J=2, shape=x_temp.shape)
plt.figure(1)
plt.subplot(5,1,1)
plt.plot(x[0])
plt.subplot(5,1,2)
...
plt.figure(2)
plt.subplot(5,1,1)
Sx = scattering(x[0])[3].reshape(1,-1)
plt.imshow(Sx, cmap='viridis', aspect='auto')
plt.colorbar()
plt.subplot(5,1,2)
Sx = scattering(x[1])[3].reshape(1,-1)
plt.imshow(Sx, cmap='viridis', aspect='auto')
plt.colorbar()
plt.subplot(5,1,3)
Sx = scattering(x[2])[3].reshape(1,-1)
plt.imshow(Sx, cmap='viridis', aspect='auto')
plt.colorbar()
plt.subplot(5,1,4)
Sx = scattering(x[3])[3].reshape(1,-1)
plt.imshow(Sx, cmap='viridis', aspect='auto')
plt.colorbar()
plt.subplot(5,1,5)
Sx = scattering(x[4])[3].reshape(1,-1)
plt.imshow(Sx, cmap='viridis', aspect='auto')
plt.colorbar()
...
if opt.args.name in non_graph:
    construct_graph(opt.args.graph_k_save_path, X_pca, y, 'heat')
...
if opt.args.name == "usps":
    ...
    lr = opt.args.lr_kyber
elif opt.args.name == "kyberpublic":
    lr = opt.args.lr_kyberpublic
elif opt.args.name == "kyberpublictemp":
    lr = opt.args.lr_kyberpublictemp
else:
    print("missing lr!")
