...
def construct_graph(fname, features, label, method='heat'):
    # fname = 'graph/kyber_graph.txt'
    num = len(label)
    dist = None
...
reut = np.loadtxt('data/kyber.txt', dtype=float)
label = np.loadtxt('data/kyber_label.txt', dtype=int)

construct_graph('graph/kyber_graph.txt', reut, label, 'ncos')
